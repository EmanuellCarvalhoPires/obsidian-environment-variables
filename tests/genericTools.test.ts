...
describe("service choices", () => {
  const note = (name: string) => ({ path: `Acesso/${name}.md`, name, frontmatter: {}, tags: [] });

  it("drops the shared part of the note names", () => {
    const ids = serviceChoices([note("Acesso Jira - Telecall"), note("Acesso Jira - A5X"), note("Acesso Jira - Assets")]).map((c) => c.id);
    expect(ids).toEqual(["A5X", "Assets", "Telecall"]);
  });

  it("keeps the full name for a single note or when ids would clash", () => {
    expect(serviceChoices([note("Acesso Jira - A5X")]).map((c) => c.id)).toEqual(["Acesso Jira - A5X"]);
  ...
  it("validates the generic tool properties", () => {
    ...
    expect(parse({ ...base, service_tag: "jira/instancia", service_param: "instancia" }).problems).toEqual([]);
    expect(parse({ ...base, service_tag: "jira/instancia" }).serviceParam).toBe("instance");
...
describe("generic tools: one service note per instance", () => {
  ...
  beforeAll(async () => {
    ...
    source.add("Acesso/Acesso Jira - Alpha.md", { tags: ["jira/instancia"], url: `http://127.0.0.1:${port}/alpha`, token_chave: "{{basic:JIRA_A}}" });
    source.add("Acesso/Acesso Jira - Beta.md", { tags: ["jira/instancia"], url: `http://127.0.0.1:${port}/beta`, token_chave: "{{basic:JIRA_B}}" });
    source.add("Acesso/Acesso Jira - Molde.md", { tags: ["jira/instancia", "molde"], url: "https://empresa.atlassian.net", token_chave: "" });
    source.add(
  ...
  it("refuses an instance that is not a note (or a template)", async () => {
    await expect(tools.run("jira_buscar_ticket", { instancia: "Molde", chave: "X" }, client)).rejects.toThrow(/must be one of: Alpha, Beta/);
  ...
  it("picks up a new instance note without touching the tools", async () => {
    source.add("Acesso/Acesso Jira - Gamma.md", { tags: ["jira/instancia"], url: `http://127.0.0.1:${port}/gamma`, token_chave: "{{basic:JIRA_A}}" });
    await registry.refresh();
