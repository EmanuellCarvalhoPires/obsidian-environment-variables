...
- The **authoring guide** comes as three prompts, one per task: **Set up the MCP environment** (everything from scratch: AI client, vault tools, environment variables, service, request and tool notes; full plan), **Add one tool** (reuses what the vault has; short plan) and **Add several tools** (shared service and request notes; full plan). Clients connected to the plugin get them through the `configure_vault_tools`, `add_vault_tool` and `add_vault_tools` MCP prompts and the `get_tool_authoring_guide` tool (`mode`: `setup`, `single` or `multiple`). You can also copy them from the panel (**Vault tools → Prompts for AI agents**) or with the **Copy the AI agent prompt** commands, and paste them into any agent.
- The prompts are generic and the same on any computer: they name no vault, folder, tag, MCP server, secret or tool of the user. The agent finds those out with `list_vault_tools` and `list_secrets`. Each prompt ends with a mandatory question (which environment variables and for which app or service; for the tool prompts, also which tools), and the agent must not create anything until it has the answer.
...
- `run_vault_tool` runs any tool by name, including tools with `expose: false` and tools created after the client loaded its list. The same is available over REST: `GET /v1/tools`, `POST /v1/tools/<name>`, `GET /v1/tools/guide?mode=setup|single|multiple`.
